...
from strategy_utils import run_strategy
...
def get_demo_data_from_yf(tickerCode, period=None, start_date=None, end_date=None):
    # get same format data for candlestick using yf
    ...
    hist["time"] = hist["Date"].dt.strftime("%Y-%m-%d")
    hist.drop(columns=["Date"], inplace=True)
    # convert all keys to lower case
    hist.columns = hist.columns.str.lower()
    logger.info(f"yfinance data downloaded for {tickerCode}. len: {len(hist)}")
    return hist.to_dict("records")


# buffer used for dynamic data
# bufferData = []
# bufferSymbol = "AAPL"
...
def update_landing_page_candlestick(
    seriesData: list[list[dict]], nIntervals: int, data: dict[list], *args
) -> list[list[dict]]:
    ...
    """
    ticker_clicked = ctx.triggered_id
    # disabled. We use store instead
    # global bufferData, bufferSymbol
    bufferSymbol = list(data.keys())[0] if data else "AAPL"
    bufferData = data[bufferSymbol] if data else []
    if (
        not ticker_clicked
        or ticker_clicked == "landing-page-candlestick-chart-interval"
    ):
        # pop & append to seriesData[0]
        if len(bufferData) < 1 or nIntervals < 1:
            return seriesData, {bufferSymbol: bufferData}
        nextData = bufferData.pop(0)
        seriesData[0].append(nextData)
        return seriesData, {bufferSymbol: bufferData}
    else:
        tickerCode = ticker_clicked.split("-")[-1].upper()
        # check if tickerCode is previous ticker
        if tickerCode == bufferSymbol:
            return seriesData, {bufferSymbol: bufferData}

        candlestickData = get_demo_data_from_yf(tickerCode, "6mo")
        bufferData = candlestickData[-30:]
        candlestickData = candlestickData[:-30]
        bufferSymbol = tickerCode
        return [candlestickData], {bufferSymbol: bufferData}
...
def tvwlcComponent():
    return html.Div(
        [
            dash_tvlwc.Tvlwc(
                id="analysis-page-candlestick-chart-2",
                seriesData=[get_demo_data_from_yf("AAPL", "5y")],
        ...
        className="zoomable w-full h-full flex justify-center items-center p-0 animate-fade-in animate-400 rounded-lg overflow-hidden",
    )
...
def dashboardPage():
    ...
    ticker_records = get_demo_data_from_yf("AAPL", None, "2015-10-15", "2019-12-31")
...
def landingPage() -> html.Div:
    ...
    """
    # global bufferData, bufferSymbol  # default is AAPL
    bufferSymbol = "AAPL"
    candlestickData = get_demo_data_from_yf(bufferSymbol, "6mo")
    # bufferdata is last 30 days
    bufferData = candlestickData[-30:]
    candlestickData = candlestickData[:-30]
    logger.info(
    ...
    return html.Div(
        [
            ...
            html.Nav(
                [
                    html.Button(
                        [html.Span("M", className="text-slate-50 text-lg"), "ETA"],
                        id="navbar-meta",
                    ...
                    html.Button(
                        [
                            html.Span("A", className="text-slate-50 text-lg"),
                            "APL",
                        ],
                        id="navbar-aapl",
                        className="text-md font-semibold text-slate-400 px-3 py-1 cursor-default hover:bg-slate-800 transition duration-300 ease-in-out rounded-lg focus:ring focus:ring-slate-800 focus:ring-opacity-50 focus:bg-slate-800",
                    ...
                    html.Button(
                        [
                            html.Span("N", className="text-slate-50 text-lg"),
                            "FLX",
                        ],
                        id="navbar-nflx",
                        className="text-md font-semibold text-slate-400 px-3 py-1 cursor-default hover:bg-slate-800 transition duration-300 ease-in-out rounded-lg focus:ring focus:ring-slate-800 focus:ring-opacity-50 focus:bg-slate-800",
                    ...
                    html.Button(
                        [
                            html.Span("G", className="text-slate-50 text-lg"),
                            "OOG",
                        ],
                        id="navbar-goog",
                        className="text-md font-semibold text-slate-400 px-3 py-1 cursor-default hover:bg-slate-800 transition duration-300 ease-in-out rounded-lg focus:ring focus:ring-slate-800 focus:ring-opacity-50 focus:bg-slate-800",
                ...
                [
                    html.Div(
                        [
                            ...
                            html.P(
                                "Backtesting on an equal-weighted portfolio of MAANG stocks from 01/01/2015 to 31/12/2019. This work is based on the project of the course FT5010 Algorithmic Trading Systems Design and Deployment.",
                        ...
                        className="h-3/5 w-full flex flex-col justify-start items-start p-4 bg-slate-900 mb-4",
                    )
                ],
